...
class Database {
  ...
  async addLocalRepository(vfsRoot, path) {
    ...
    indexPath += arch + '/';
    const file = vfs.path(vfsRoot, (indexPath + 'APKINDEX.tar.gz').split('/').filter(n => n));

    await this.loadRepository(file.content, indexPath);
  }
  ...
  async addRepository(repo) {
    ...
    const buffer = await download(url + 'APKINDEX.tar.gz');
    await this.loadRepository(buffer, url);
  }

  async loadRepository(buffer, url) {
...
class InstallList {
  ...
  async download(vfsCacheDirectory) {
    for(const pack of this._list) {
      ...
      const url = `${pack.repo}${pack.name}-${pack.version}.apk`;
      const buffer = await download(url);
      if(buffer.length !== pack.size) {
